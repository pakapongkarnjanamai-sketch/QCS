<#
.SYNOPSIS
    Sets up QCS IIS applications and app pools on the Production web server (AP-NTC2137-PRWB).
...
.NOTES
    Prerequisites:
      - .NET 9 ASP.NET Core Hosting Bundle must be installed on this server.
        Download: https://dotnet.microsoft.com/download/dotnet/9.0
        After installing, run: iisreset
      - IIS features: Windows Authentication, URL Rewrite module.
#>
...
# ── Main ────────────────────────────────────────────────────────────────
Write-Host "`n=== QCS Production IIS Setup (AP-NTC2137-PRWB) ===" -ForegroundColor Yellow
...
Write-Host "`n=== QCS Production IIS Setup Complete ===" -ForegroundColor Green
Write-Host "Next steps:" -ForegroundColor Green
Write-Host "  1. Ensure .NET 9 ASP.NET Core Hosting Bundle is installed (dotnet --list-runtimes)" -ForegroundColor Green
Write-Host "  2. Run Deploy-PR.ps1 from your dev machine to publish and copy application files." -ForegroundColor Green
