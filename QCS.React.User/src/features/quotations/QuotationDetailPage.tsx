...
export function QuotationDetailPage() {
  ...
  return <div className="mx-auto grid max-w-6xl gap-6"><section className="rounded-sm border border-border-subtle bg-surface-panel p-4 md:p-6"><div className="flex flex-wrap items-center gap-2"><h1 className="text-title font-semibold">{request.code}</h1><StatusBadge status={request.statusName} /></div><p className="mt-1 text-heading text-ink-muted">{request.title}</p>{request.sourceSystem === 'QRS' && request.sourceCode && <a href={qrsRequestUrl(request.sourceCode)} target="_blank" rel="noreferrer" className="mt-4 inline-flex items-center gap-2 rounded-sm text-body text-accent hover:underline focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent">Open source request {request.sourceCode}<ExternalLink size={15} aria-hidden /></a>}{finalDocument ? <div className="mt-6"><h2 className="text-heading font-semibold">Stamped final PDF</h2><div className="mt-3"><DocumentList documents={[finalDocument]} onPreview={setPreview} /></div></div> : <div className="mt-6 border-l-2 border-border-subtle pl-3 text-body text-ink-muted">A stamped final PDF is not available.</div>}</section>{error && <ErrorSurface>{error.title} Showing the previous details.</ErrorSurface>}<section className="rounded-sm border border-border-subtle bg-surface-panel p-4 md:p-6"><h2 className="text-heading font-semibold">Documents</h2><div className="mt-4"><DocumentList documents={request.documents} onPreview={setPreview} /></div></section><PdfViewer document={preview} onClose={() => setPreview(undefined)} /></div>
}
