...
export function RequestDetailPage() {
  ...
  return <div className="mx-auto grid max-w-6xl gap-6"><Link to={backTo} className="inline-flex w-fit items-center gap-2 rounded-sm text-body text-accent hover:underline focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent"><ArrowLeft size={16} aria-hidden />Back to requests</Link>{error && <ErrorSurface>{error.title} Showing the previous details.</ErrorSurface>}<section className="rounded-sm border border-border-subtle bg-surface-panel p-4 md:p-6"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h1 className="break-words text-title font-semibold">{request.code}</h1><StatusBadge status={request.statusName} /></div><p className="mt-1 break-words text-heading text-ink-muted">{request.title}</p></div><p className="text-body text-ink-muted">{formatDate(request.requestDate)}</p></div><dl className="mt-6 grid gap-x-6 gap-y-4 border-t border-border-subtle pt-4 sm:grid-cols-2 lg:grid-cols-3"><Detail label="Requester" value={request.requesterName || request.requesterNId} /><Detail label="Vendor" value={request.vendorName || request.vendorCode} /><Detail label="Current step" value={request.currentStepName || '-'} /><Detail label="Valid from" value={formatDate(request.validFrom)} /><Detail label="Valid until" value={formatDate(request.validUntil)} />{request.remark && <Detail label="Remark" value={request.remark} />}</dl>{request.sourceSystem === 'QRS' && request.sourceCode && <a href={qrsRequestUrl(request.sourceCode)} target="_blank" rel="noreferrer" className="mt-5 inline-flex items-center gap-2 rounded-sm text-body text-accent hover:underline focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent">Open source request {request.sourceCode}<ExternalLink size={15} aria-hidden /></a>}</section><div className="grid gap-6 lg:grid-cols-[minmax(0,1.4fr)_minmax(18rem,1fr)]"><section className="rounded-sm border border-border-subtle bg-surface-panel p-4 md:p-6"><h2 className="text-heading font-semibold">Documents</h2><div className="mt-4"><DocumentList documents={request.documents} onPreview={setPreview} /></div></section><section className="rounded-sm border border-border-subtle bg-surface-panel p-4 md:p-6"><h2 className="text-heading font-semibold">Workflow</h2><div className="mt-4"><WorkflowTimeline steps={request.workflowSteps} /></div></section></div><section className="rounded-sm border border-border-subtle bg-surface-panel p-4 md:p-6"><h2 className="text-heading font-semibold">History</h2><div className="mt-4"><HistoryList histories={request.histories} /></div></section><PdfViewer document={preview} onClose={() => setPreview(undefined)} /></div>
}
