...
export function ApprovalActionDialog({ action, busy, onClose, onConfirm }: { action?: 'approve' | 'reject'; busy: boolean; onClose: () => void; onConfirm: (comment: string) => void }) {
  ...
  return <Modal open={Boolean(action)} title={label} onClose={onClose}><div className="grid gap-5"><label className="block"><span className="mb-1 block text-caption font-medium uppercase tracking-[0.08em] text-ink-muted">Comment {reasonRequired ? '(required)' : '(optional)'}</span><textarea autoFocus value={comment} onChange={(event) => setComment(event.target.value)} rows={4} className={appTextareaClassName('w-full')} /></label><div className="flex justify-end gap-2"><AppButton tone="secondary" onClick={onClose} disabled={busy}>Cancel</AppButton><AppButton tone={action === 'reject' ? 'danger' : 'primary'} onClick={() => onConfirm(trimmed)} disabled={busy || (reasonRequired && !trimmed)}>{busy ? 'Working...' : action === 'approve' ? 'Approve' : 'Reject'}</AppButton></div></div></Modal>
}