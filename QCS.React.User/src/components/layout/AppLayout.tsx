...
export function AppLayout({ children }: { children: ReactNode }) {
  ...
  const sidebar = <aside className={`fixed inset-y-0 left-0 z-40 flex w-56 flex-col border-r border-border-subtle bg-white transition-transform md:static md:translate-x-0 ${sidebarWidth} ${mobileOpen ? 'translate-x-0' : '-translate-x-full'}`}>
    ...
    <nav className="grid gap-1 p-2">{navigation.map(({ icon: Icon, label, path }) => <NavLink key={path} to={path} end={path === '/'} onClick={() => setMobileOpen(false)} title={collapsed ? label : undefined} className={({ isActive }) => `flex items-center gap-3 rounded-sm px-3 py-2 text-body focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent ${isActive ? 'bg-accent-soft text-accent' : 'text-ink-muted hover:bg-surface-muted hover:text-ink-strong'} ${collapsed ? 'md:justify-center md:px-2' : ''}`}><Icon size={18} /><span className={collapsed ? 'md:hidden' : ''}>{label}</span></NavLink>)}</nav>
  </aside>
